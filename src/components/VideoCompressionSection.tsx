...
import { compressVideo, downloadBlob, CompressionResult } from "@/lib/compression";
import { useToast } from "@/hooks/use-toast";
import { Video } from "lucide-react";

export const VideoCompressionSection = () => {
  ...
  const handleCompress = async () => {
    ...
    try {
      const compressionResult = await compressVideo(selectedFile, algorithm);
      
      setResult(compressionResult);
      toast({
        title: "Compression successful!",
        description: `Video compressed by ${compressionResult.compressionRatio.toFixed(1)}%`,
      });
    } catch (error) {
      toast({
  ...
  return (
    <div className="animate-fade-in">
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl font-bold text-foreground mb-2">Video Compression</h2>
        <p className="text-muted-foreground">
          Compress your videos using H.264 codec with optimized settings
        </p>
      </div>
